import { createDistanceMap, findShortestPath } from '../lib/index.js'
...
class Model {
    ...
    @computed get fleeCoefficientTargetsMap() {
        const map = this.baseDistanceMap
        return new Map(Array.from(map.entries()).map(([tile, cost]) => [tile, -Math.floor(cost * 1.8)]));
    }

    @computed get fleeCoefficientDistanceMap() {
        const map = this.baseDistanceMap
        const targets = Array.from(map.entries()).map(([tile, cost]) => ({ node: tile, cost: -Math.floor(cost * 1.8) }));
        return createDistanceMap(this.getNeighbours, targets);
    }
        ...
        <div style={{maxWidth: 800, margin: 'auto'}}>
            ...
            <div style={rowStyle}>
                <div>
                    ...
                    an agent can reach, and what the cost will be. The argument <code>node</code> is <code>NodeType</code> and returns an array of transition 
                    descriptions in the shape <code>{`{ node, cost }`}</code></p>
...
createDistanceMap(getNeighbours, targets);`}
                    ...
                    <p><code>findShortestPath(distanceMap, startNode, getNeighbours)</code></p>
                    ...
                    <p>Once you have a distance map you can pass it to <code>findShortestPath</code> to find 
                       ...
                       The array does not include the startNode itself. 
                    </p>
                    ...
                    <div>
                        ...
                        But in order to make the distance map a bit more clever we can 
                            ...
                            <label style={{ display: 'inline-block', marginBottom: 0 }}>
                                ...
                                multiply this cost with a coefficient (1.8)&nbsp;
                            </label>

                        and use this new data 
                            <label style={{ display: 'inline-block', marginBottom: 0 }}>
